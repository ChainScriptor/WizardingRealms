...
const SphereImageGrid: React.FC<SphereImageGridProps> = ({
  ...
  const renderSpotlightModal = () => {
    ...
    return (
      ...
      >
        <div
          className="bg-zinc-950 text-white rounded-xl max-w-md w-full overflow-hidden ring-1 ring-zinc-800"
        ...
        >
          <div className="relative aspect-square">
            ...
            <button
              onClick={() => setSelectedImage(null)}
              className="absolute top-2 right-2 w-8 h-8 bg-black/60 rounded-full text-white flex items-center justify-center hover:bg-black/80 transition-all cursor-pointer"
            >
              <X size={16} />
            </button>
          </div>
  ...
  if (!isMounted) {
    return (
      <div
        className="bg-zinc-900 rounded-lg animate-pulse flex items-center justify-center"
  ...
  if (!images.length) {
    return (
      <div
        className="bg-zinc-900/60 rounded-lg border-2 border-dashed border-zinc-700 flex items-center justify-center"
        style={{ width: containerSize, height: containerSize }}
      >
